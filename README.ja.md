...
コマンドは **27** 個で、`doctor` 以外はローカルの Web ページからも使えます。テストは Python が **1,391** 件、ページが **132** 件、行と分岐の網羅率は **100%**。同一性は **595** 枚のイメージ、空ディスクの値は一度も書き換えられていない **1,729** 面から測定しています。
...
根拠となる測定値を添えた評価。`--read` は繰り返し吸い出しを反映し、`--captures` は保存したキャプチャ一式が示す弱いブロックを数えます。`--map` はブロックごとの信頼度と、その根拠を表示します。弱いブロックが 1 つでもあれば評価は marginal に留まります。読み取りごとにパルスが揺れるブロックこそ、次に壊れるブロックだからです。ディスク情報がイメージと異なるキャプチャは、別のディスクを表すため拒否します。
...
同一の物理ディスクを繰り返し吸い出した結果をブロック単位で比較します。安定度、どのブロックが揺れるか、ビット反転の向きを報告します。磁気的な減衰は磁化の反転を失う方向に働くため、1 が 0 へ落ちます。本ツールはこれを `loss`、逆方向を `gain`、両方を `mixed` と呼びます。ブロックは種別とファイル番号で照合するため、あるブロックを取りこぼした吸い出しは拒否されず、取りこぼしとして数えられます。
...
1 枚のディスクの吸い出しを複数受け取り、ブロックごとの多数決で 1 つのイメージに統合し、吸い出しどうしが一致しなかったブロックをすべて報告します。`--map` はブロックごとの一致度を表示します。ブロックは位置ではなく種別とファイル番号で照合するため、あるブロックを取りこぼした吸い出しも統合に加わります。そのブロックはほかの吸い出しが決め、取りこぼした吸い出しの数を報告します。`.qd` の吸い出しのようにチェックサムを持つ場合は、チェックサムが一致しない多数派よりも、チェックサムが一致する 1 つの写しを優先し、判定は `checksum` となります。

`--captures` は、保存したキャプチャ一式からパルス投票でディスクを再構成します。どの読み取りも正しく読めなかったブロックについて、そこに届いた各読み取りのパルスを 1 つずつ比べ、多数派を残します。パルスが 1 つ増えたり減ったりした読み取りは、投票の前にほかの読み取りへ位置を合わせ直すため、1 つのずれがその後のパルスすべてをずらすことはありません。`reads` もそのずれを、ブロック全体の不一致ではなく 1 回として数えます。そのブロックの読み取りが 3 回必要で、すべての読み取りが同じ場所を読み違えたブロックは直りません。共通の誤りを投票で覆すことはできないからです。単独で使うと再構成したディスクが出力となり、直せなかったブロックをすべて列挙します。吸い出しと合わせると、再構成したディスクが投票者の 1 つとして加わります。
...
ディスクを読み取り、出力のファイル拡張子に応じて `.fds` または `.qd` として保存します。`.qd` は各ブロックがディスク上で持っていたチェックサムを保つため、一度も正しく読めなかったブロックはその中でも不良のまま見分けられます。`--passes` は各面を複数回読み、`--retries` は不良ブロックのある面をあと何回まで読み直すかを決め、`--raw` はドライブが返したパルスキャプチャをすべて、ほかのコマンドが読み戻せる一式として保存します。吸い出しが失敗したときや中断したときも、この一式は保存されます。キャプチャが意味を持つのは、まさに読めなくなりつつあるディスクだからです。USB 接続がパルスデータのパケットを取りこぼした場合も、吸い出しはそのことを伝えます。失われたパケットの周辺のブロックは、ディスクとは無関係の理由で失敗するからです。
...
すべての読み取りと書き込みには期限があります。まだ何も測定していない段階では 1 面につき 20 秒、その後は、直近 4 面のうち最も時間のかかった面の 3 倍を上限とし、2 秒を下回ることはありません。1 回だけ速く読めても、次の面の時間が足りなくなることはありません。期限を過ぎた面はコマンドを止め、デバイスを閉じ、再試行もしません。一度止まったドライブで続けても、ディスクを傷めるだけだからです。止まった吸い出しはファイルを書き出しません。止まった書き込みは、面が書きかけの可能性があると伝えます。何が書き込まれたかは、改めて吸い出さなければ分からないからです。アダプタが期待するレートなら 1 面は約 5.5 秒で読めます。これらの上限は実測したドライブではなくこの数値から決めたもので、実機で最初に見直すべき値です。
...
報告は、失敗がどこにあるかも示します。摩耗は面の終わりに集中するため、面の最後の 3 分の 1 にある失敗は摩耗を、1 つだけ失敗したブロックは 1 か所の損傷を示し、それ以外は分散と呼びます。失敗したテストの最後には、損傷と判断する前に軽く清掃してもう一度実行するよう勧めます。汚れも摩耗した磁性面と同じようにブロックを失敗させ、清掃しても失われたデータは戻らないからです。
...
- ドライブが止まった場合や Ctrl-C を押した場合は即座に止まり、それでも終わったパスはすべて報告に残ります。
...
`--bracket` は、修理ガイドがヘッドを合わせるのに使う方法を実行します。ディスクが読めなくなるまで動かし、反対側でも読めなくなるまで戻し、その中央に落ち着かせます。最初の読み取り以降、読み取りのたびに小さく 1 段、毎回同じ大きさだけ回すよう求めます。ヘッドのねじなら 8 分の 1 回転です。ディスクが読めなくなったところで向きを変えるよう伝え、反対側でも読めなくなったら、読めた範囲の幅と、その中央まで何段戻すかを示します。これは毎回同じ大きさで回した場合にだけ成り立つので、回した回数を数えてください。1 回の読み取りの失敗では境界とみなしません。回さずにもう一度読むよう求め、同じ設定で 2 回続けて失敗したときだけ境界とするため、たまたま失敗した 1 回で範囲が狭まることはありません。
...
3. ベルトを交換した後は `calibrate head` を実行し、スピンドルハブ、次にヘッドをおよそ 45 度ずつ調整して、すべての読み取りが正しくなるまで続けます。
