...
def _intervals(path: Path, fmt: CaptureFormat | None) -> tuple[int, ...]:
    ...
    if capture.quantised:
        message = (
            ...
            "Run classes on it instead, which reads what this capture can answer."
        )
        raise fail(message)
...
def classes(
    capture: Annotated[Path, typer.Argument(help="a capture of pulse classes")],
    fmt: Annotated[
        CaptureFormat | None,
        typer.Option("--format", help="override the detected capture format"),
    ] = None,
    *,
    ...
    """Judge a capture that carries pulse classes rather than pulse timing."""
    if not capture.is_file():
        message = f"file not found: {capture}"
        raise fail(message)

    data = capture.read_bytes()
    packed = (fmt or CaptureFormat.RAW03) is CaptureFormat.RAW03
    try:
        report = measure_classes(data, packed=packed)
    except ValueError as error:
        raise fail(str(error)) from error
...
def reading(
    ...
    *,
    ...
    """Read a console tool's cycles-between-bytes figure and say what to turn."""
    try:
    ...
    turn = {
        Direction.FASTER: "counter-clockwise",
        Direction.SLOWER: "clockwise",
        Direction.HOLD: "nothing",
    }[report.direction]
    ...
    typer.echo(report.render())
    if report.direction is Direction.HOLD:
        typer.echo("leave the trimmer alone")
    else:
        typer.echo(f"turn the motor trimmer {turn} to run {report.direction.value}")
    raise typer.Exit(code=0 if report.verdict is Verdict.FINE else 1)
